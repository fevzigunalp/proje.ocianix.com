...
export default function ProjectDeleteConfirm({ project, onClose, onConfirm }) {
  const [stage, setStage] = useState(1); // 1 = ilk onay, 2 = "SİL" yazma
      ...
      <div className="bg-surface dark:bg-surface-dark-alt border border-border-light dark:border-border-dark rounded-2xl w-full max-w-md" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-start justify-between p-5 border-b border-border-light dark:border-border-dark">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h2 className="text-base font-bold">Projeyi Sil</h2>
              <p className="text-xs text-text-muted mt-0.5 break-words">{project.name}</p>
            </div>
          </div>
        ...
        {stage === 1 && (
          <div className="p-5 space-y-4">
            <p className="text-sm leading-relaxed">
              <strong>Bu projeyi silmek istediğine emin misin?</strong>
            </p>
